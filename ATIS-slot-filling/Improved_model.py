...
from keras.layers import (Input, Embedding, SimpleRNN, Dense, Activation,
                          TimeDistributed, Convolution1D, Dropout, GRU, LSTM)
...
model = Sequential()
model.add(Embedding(vocsize, 100))
model.add(Convolution1D(128, 5, border_mode='same', activation='relu'))     # implementation of lookahead
model.add(Dropout(0.25))                                                    # drop out with 25% chance
model.add(GRU(100, return_sequences=True))                                  # gru worked best at this type of model (than simpleRNN, LSTM)
...
for e in range(s['nepochs']):
    ...
    for i in range(nsentences):
        ...
        if X.shape[1] == 1:     # skip iteration due to possible bug
            continue
        model.train_on_batch(X, Y)                      # train on batch with X(input sentence) and Y(labels)
